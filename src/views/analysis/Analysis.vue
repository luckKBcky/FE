...
const getData = async () => {
  try {
    const response = await axios.get(
      `https://t1115.p.ssafy.io/ai/getJSON?month=${month.value}`
    );
  ...
  sum.value =
    data.value.etc.amount +
    data.value.food.amount +
    data.value.leisure.amount +
    data.value.beauty.amount +
    data.value.allowance.amount +
    data.value.clothing.amount +
    data.value.dating.amount +
    data.value.savings.amount +
    data.value.studies.amount +
    data.value.convenience.amount;
};
    ...
    <div class="white-box rank-white-box">
      ...
      <div class="white-box-content white-box-content-card">
        <div
          ...
          :key="index"
          :style="{ 'background-color': index == 0 ? '#E1EDFD' : '#F6F9F8' }"
        >
          <div
            class="ranking-box"
            :style="{ 'background-color': index == 0 ? '#4483F7' : '#7F8591' }"
          >
            {{ index + 1 }}위
          </div>
          <div>
            ...
            <HeadText
              :header-text="formatNumberWithComma(item.amount) + '원'"
            />
          </div>
          <div class="percent-box">
            {{ formatNumber((item.amount / sum) * 100) }}%
          </div>
        </div>
      </div>
    </div>
